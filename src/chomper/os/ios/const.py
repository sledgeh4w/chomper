# System call numbers

SYS_SYSCALL = 0x0
SYS_EXIT = 0x1
SYS_FORK = 0x2
SYS_READ = 0x3
SYS_WRITE = 0x4
SYS_OPEN = 0x5
SYS_CLOSE = 0x6
SYS_LINK = 0x9
SYS_UNLINK = 0xA
SYS_CHDIR = 0xC
SYS_FCHDIR = 0xD
SYS_CHMOD = 0xF
SYS_CHOWN = 0x10
SYS_GETPID = 0x14
SYS_SETUID = 0x17
SYS_GETUID = 0x18
SYS_GETEUID = 0x19
SYS_PTRACE = 0x1A
SYS_SENDMSG = 0x1C
SYS_RECVFROM = 0x1D
SYS_GETPEERNAME = 0x1F
SYS_GETSOCKNAME = 0x20
SYS_ACCESS = 0x21
SYS_CHFLAGS = 0x22
SYS_FCHFLAGS = 0x23
SYS_KILL = 0x25
SYS_GETPPID = 0x27
SYS_DUP = 0x29
SYS_PIPE = 0x2A
SYS_GETEGID = 0x2B
SYS_SIGACTION = 0x2E
SYS_GETGID = 0x2F
SYS_SIGPROCMASK = 0x30
SYS_GETLOGIN = 0x31
SYS_SETLOGIN = 0x32
SYS_SIGALTSTACK = 0x35
SYS_IOCTL = 0x36
SYS_REBOOT = 0x37
SYS_SYMLINK = 0x39
SYS_READLINK = 0x3A
SYS_EXECVE = 0x3B
SYS_CHROOT = 0x3D
SYS_MSYNC = 0x41
SYS_MUNMAP = 0x49
SYS_MPROTECT = 0x4A
SYS_MADVISE = 0x4B
SYS_SETPGID = 0x52
SYS_SETITIMER = 0x53
SYS_GETITIMER = 0x56
SYS_GETDTABLESIZE = 0x59
SYS_DUP2 = 0x5A
SYS_FCNTL = 0x5C
SYS_SELECT = 0x5D
SYS_FSYNC = 0x5F
SYS_SETPRIORITY = 0x60
SYS_SOCKET = 0x61
SYS_CONNECT = 0x62
SYS_GETPRIORITY = 0x64
SYS_BIND = 0x68
SYS_SETSOCKOPT = 0x69
SYS_LISTEN = 0x6A
SYS_SIGSUSPEND = 0x6F
SYS_GETTIMEOFDAY = 0x74
SYS_GETRUSAGE = 0x75
SYS_GETSOCKOPT = 0x76
SYS_READV = 0x78
SYS_WRITEV = 0x79
SYS_SETTIMEOFDAY = 0x7A
SYS_FCHOWN = 0x7B
SYS_FCHMOD = 0x7C
SYS_RENAME = 0x80
SYS_FLOCK = 0x83
SYS_SENDTO = 0x85
SYS_SHUTDOWN = 0x86
SYS_SOCKETPAIR = 0x87
SYS_MKDIR = 0x88
SYS_RMDIR = 0x89
SYS_UTIMES = 0x8A
SYS_FUTIMES = 0x8B
SYS_ADJTIME = 0x8C
SYS_SETSID = 0x93
SYS_GETPGID = 0x97
SYS_PREAD = 0x99
SYS_PWRITE = 0x9A
SYS_QUOTACTL = 0xA5
SYS_CSOPS = 0xA9
SYS_CSOPS_AUDITTOKEN = 0xAA
SYS_GETRLIMIT = 0xC2
SYS_SETRLIMIT = 0xC3
SYS_MMAP = 0xC5
SYS_LSEEK = 0xC7
SYS_FTRUNCATE = 0xC9
SYS_SYSCTL = 0xCA
SYS_MLOCK = 0xCB
SYS_MUNLOCK = 0xCC
SYS_OPEN_DPROTECTED_NP = 0xD8
SYS_GETATTRLIST = 0xDC
SYS_SETATTRLIST = 0xDD
SYS_FSETATTRLIST = 0xE5
SYS_SETXATTR = 0xEC
SYS_FSETXATTR = 0xED
SYS_LISTXATTR = 0xF0
SYS_POSIX_SPAWN = 0xF4
SYS_SHM_OPEN = 0x10A
SYS_SYSCTLBYNAME = 0x112
SYS_GETTID = 0x11E
SYS_IDENTITYSVC = 0x125
SYS_PSYNCH_MUTEXWAIT = 0x12D
SYS_PSYNCH_CVWAIT = 0x131
SYS_PROCESS_POLICY = 0x143
SYS_MLOCKALL = 0x144
SYS_ISSETUGID = 0x147
SYS_PTHREAD_KILL = 0x148
SYS_PTHREAD_SIGMASK = 0x149
SYS_DISABLE_THREADSIGNAL = 0x14B
SYS_SEMWAIT_SIGNAL = 0x14E
SYS_PROC_INFO = 0x150
SYS_SENDFILE = 0x151
SYS_STAT64 = 0x152
SYS_FSTAT64 = 0x153
SYS_LSTAT64 = 0x154
SYS_GETDIRENTRIES64 = 0x158
SYS_STATFS64 = 0x159
SYS_FSTATFS64 = 0x15A
SYS_FSSTAT64 = 0x15B
SYS_BSDTHREAD_CREATE = 0x168
SYS_KQUEUE = 0x16A
SYS_KEVENT = 0x16B
SYS_LCHOWN = 0x16C
SYS_WORKQ_OPEN = 0x16F
SYS_WORKQ_KERNRETURN = 0x170
SYS_THREAD_SELFID = 0x174
SYS_KEVENT_QOS = 0x176
SYS_KEVENT_ID = 0x177
SYS_MAC_SYSCALL = 0x17D
SYS_READ_NOCANCEL = 0x18C
SYS_WRITE_NOCANCEL = 0x18D
SYS_OPEN_NOCANCEL = 0x18E
SYS_CLOSE_NOCANCEL = 0x18F
SYS_SENDMSG_NOCANCEL = 0x192
SYS_RECVFROM_NOCANCEL = 0x193
SYS_FCNTL_NOCANCEL = 0x196
SYS_SELECT_NOCANCEL = 0x197
SYS_FSYNC_NOCANCEL = 0x198
SYS_CONNECT_NOCANCEL = 0x199
SYS_READV_NOCANCEL = 0x19B
SYS_WRITEV_NOCANCEL = 0x19C
SYS_SENDTO_NOCANCEL = 0x19D
SYS_PREAD_NOCANCEL = 0x19E
SYS_PWRITE_NOCANCEL = 0x19F
SYS_SEMWAIT_SIGNAL_NOCANCEL = 0x1A7
SYS_GUARDED_OPEN_NP = 0x1B9
SYS_GUARDED_CLOSE_NP = 0x1BA
SYS_GETATTRLISTBULK = 0x1CD
SYS_CLONEFILEAT = 0x1CE
SYS_OPENAT = 0x1CF
SYS_OPENAT_NOCANCEL = 0x1D0
SYS_RENAMEAT = 0x1D1
SYS_FACCESSAT = 0x1D2
SYS_FCHMODAT = 0x1D3
SYS_FCHOWNAT = 0x1D4
SYS_FSTATAT64 = 0x1D6
SYS_LINKAT = 0x1D7
SYS_UNLINKAT = 0x1D8
SYS_READLINKAT = 0x1D9
SYS_SYMLINKAT = 0x1DA
SYS_MKDIRAT = 0x1DB
SYS_BSDTHREAD_CTL = 0x1DE
SYS_GUARDED_PWRITE_NP = 0x1E6
SYS_RENAMEATX_NP = 0x1E8
SYS_PERSONA = 0x1EE
SYS_GETENTROPY = 0x1F4
SYS_NECP_OPEN = 0x1F5
SYS_ULOCK_WAIT = 0x203
SYS_FCLONEFILEAT = 0x205
SYS_TERMINATE_WITH_PAYLOAD = 0x208
SYS_ABORT_WITH_PAYLOAD = 0x209
SYS_OS_FAULT_WITH_PAYLOAD = 0x211
SYS_MEMORYSTATUS_AVAILABLE_MEMORY = 0x216
SYS_PREADV = 0x21C
SYS_PREADV_NOCANCEL = 0x21E

MACH_ABSOLUTE_TIME_TRAP = -0x3
KERNELRPC_MACH_VM_ALLOCATE_TRAP = -0xA
KERNELRPC_MACH_VM_PURGABLE_CONTROL_TRAP = -0xB
KERNELRPC_MACH_VM_DEALLOCATE_TRAP = -0xC
KERNELRPC_MACH_VM_PROTECT_TRAP = -0xE
KERNELRPC_MACH_VM_MAP_TRAP = -0xF
KERNELRPC_MACH_PORT_ALLOCATE_TRAP = -0x10
KERNELRPC_MACH_PORT_DEALLOCATE_TRAP = -0x12
KERNELRPC_MACH_PORT_MOD_REFS_TRAP = -0x13
KERNELRPC_MACH_PORT_INSERT_RIGHT = -0x15
KERNELRPC_MACH_PORT_INSERT_MEMBER_TRAP = -0x16
KERNELRPC_MACH_PORT_EXTRACT_MEMBER_TRAP = -0x17
KERNELRPC_MACH_PORT_CONSTRUCT_TRAP = -0x18
KERNELRPC_MACH_PORT_DESTRUCT_TRAP = -0x19
MACH_REPLY_PORT_TRAP = -0x1A
THREAD_SELF_TRAP = -0x1B
TASK_SELF_TRAP = -0x1C
HOST_SELF_TRAP = -0x1D
MACH_MSG_TRAP = -0x1F
SEMAPHORE_SIGNAL_TRAP = -0x21
SEMAPHORE_WAIT_TRAP = -0x24
KERNELRPC_MACH_PORT_GUARD_TRAP = -0x29
MAP_FD_TRAP = -0x2B
THREAD_GET_SPECIAL_REPLY_PORT = -0x32
HOST_CREATE_MACH_VOUCHER_TRAP = -0x46
KERNELRPC_MACH_PORT_TYPE_TRAP = -0x4C
KERNELRPC_MACH_PORT_REQUEST_NOTIFICATION_TRAP = -0x4D
MACH_TIMEBASE_INFO_TRAP = -0x59
MK_TIMER_CREATE_TRAP = -0x5B
MK_TIMER_ARM = -0x5D

# Error codes

EPERM = 1
ENOENT = 2
EBADF = 9
EACCES = 13
EFAULT = 14
EEXIST = 17
ENOTDIR = 20
EINVAL = 22
EMFILE = 24
ETIMEDOUT = 60

# device/device_types.h

IOKIT_SERVER_VERSION = 20210810

# net/route.h

RTM_VERSION = 5

# Message types.

RTM_NEWADDR = 0xC
RTM_IFINFO = 0xE
RTM_IFINFO2 = 0x12

# Bitmask values for rtm_addrs.

RTA_NETMASK = 0x4
RTA_IFP = 0x10
RTA_IFA = 0x20

# sys/attr.h

ATTR_CMN_MODTIME = 0x00000400
ATTR_CMN_ACCTIME = 0x00001000

# sys/proc.h

# Process status values

SRUN = 2

# Process flags

P_LP64 = 0x00000004

# sys/proc_info.h

# Flavors for proc_pidinfo()

PROC_PIDTBSDINFO = 3
PROC_PIDPATHINFO = 11
PROC_PIDT_SHORTBSDINFO = 13
PROC_PIDUNIQIDENTIFIERINFO = 17

# sys/resource.h

# RESOURCE LIMITS

RLIM_INFINITY = (1 << 63) - 1

RLIMIT_CPU = 0
RLIMIT_FSIZE = 1
RLIMIT_DATA = 2
RLIMIT_STACK = 3
RLIMIT_CORE = 4
RLIMIT_AS = 5
RLIMIT_RSS = RLIMIT_AS
RLIMIT_MEMLOCK = 6
RLIMIT_NPROC = 7
RLIMIT_NOFILE = 8

# sys/socket.h

# Address families.

AF_INET = 2
AF_LINK = 18
AF_INET6 = 30

# Protocol families.

PF_ROUTE = 17

# PF_ROUTE - Routing table

NET_RT_IFLIST = 3

# sys/stdio.h

RENAME_SECLUDE = 0x00000001
RENAME_SWAP = 0x00000002
RENAME_EXCL = 0x00000004

# sys/sysctl.h

# CTL Types

CTL_UNSPEC = 0
CTL_KERN = 1
CTL_VM = 2
CTL_VFS = 3
CTL_NET = 4
CTL_DEBUG = 5
CTL_HW = 6
CTL_MACHDEP = 7
CTL_USER = 8
CTL_MAXID = 9

# CTL_KERN identifiers

KERN_OSTYPE = 1
KERN_OSRELEASE = 2
KERN_OSREV = 3
KERN_VERSION = 4
KERN_MAXVNODES = 5
KERN_MAXPROC = 6
KERN_ARGMAX = 8
KERN_HOSTNAME = 10
KERN_HOSTID = 11
KERN_CLOCKRATE = 12
KERN_PROC = 14
KERN_NGROUPS = 18
KERN_SAVED_IDS = 20
KERN_BOOTTIME = 21
KERN_NISDOMAINNAME = 22
KERN_MAXFILESPERPROC = 29
KERN_USRSTACK32 = 35
KERN_USRSTACK64 = 59
KERN_OSVERSION = 65

# CTL_HW identifiers

HW_MACHINE = 1
HW_MODEL = 2
HW_NCPU = 3
HW_BYTEORDER = 4
HW_PHYSMEM = 5
HW_USERMEM = 6
HW_PAGESIZE = 7
HW_VECTORUNIT = 13
HW_CACHELINE = 16
HW_L1ICACHESIZE = 17
HW_L1DCACHESIZE = 18
HW_L2SETTINGS = 19
HW_L2CACHESIZE = 20
HW_TB_FREQ = 23
HW_MEMSIZE = 24
HW_AVAILCPU = 25
HW_TARGET = 26
HW_PRODUCT = 27

# KERN_PROC subtypes

KERN_PROC_PID = 1

# mach/arm/thread_status.h

# Flavors

ARM_THREAD_STATE64 = 6

# mach/host_info.h

HOST_BASIC_INFO = 1
HOST_PREFERRED_USER_ARCH = 12

HOST_VM_INFO = 2

HOST_VM_INFO64 = 4

# mach/host_special_ports.h

HOST_PORT = 1

# mach/kern_return.h

KERN_SUCCESS = 0
KERN_INVALID_ADDRESS = 1
KERN_RESOURCE_SHORTAGE = 6

# mach/machine.h

CPU_ARCH_ABI64 = 0x01000000

CPU_TYPE_ARM = 12
CPU_TYPE_ARM64 = CPU_TYPE_ARM | CPU_ARCH_ABI64

CPU_SUBTYPE_ARM64_ALL = 0

# mach/message.h

MACH_MSGH_BITS_ZERO = 0x00000000
MACH_MSGH_BITS_REMOTE_MASK = 0x0000001F
MACH_MSGH_BITS_LOCAL_MASK = 0x00001F00
MACH_MSGH_BITS_VOUCHER_MASK = 0x001F0000
MACH_MSGH_BITS_PORTS_MASK = (
    MACH_MSGH_BITS_REMOTE_MASK | MACH_MSGH_BITS_LOCAL_MASK | MACH_MSGH_BITS_VOUCHER_MASK
)
MACH_MSGH_BITS_COMPLEX = 0x80000000

MACH_SEND_MSG = 0x00000001
MACH_RCV_MSG = 0x00000002

MACH_RCV_TIMED_OUT = 0x10004003

MACH_MSG_PORT_DESCRIPTOR = 0
MACH_MSG_OOL_DESCRIPTOR = 1
MACH_MSG_OOL_PORTS_DESCRIPTOR = 2
MACH_MSG_OOL_VOLATILE_DESCRIPTOR = 3
MACH_MSG_GUARDED_PORT_DESCRIPTOR = 4

MACH_MSG_TYPE_MOVE_RECEIVE = 16
MACH_MSG_TYPE_MOVE_SEND = 17

# mach/port.h

MACH_PORT_TYPE_NONE = 0
MACH_PORT_TYPE_SEND = 1 << (0 + 16)
MACH_PORT_TYPE_RECEIVE = 1 << (1 + 16)
MACH_PORT_TYPE_SEND_ONCE = 1 << (2 + 16)
MACH_PORT_TYPE_PORT_SET = 1 << (3 + 16)
MACH_PORT_TYPE_DEAD_NAME = 1 << (4 + 16)
MACH_PORT_TYPE_LABELH = 1 << (5 + 16)

# mach/task_info.h

TASK_AUDIT_TOKEN = 15

# mach/task_special_ports.h

TASK_BOOTSTRAP_PORT = 4
TASK_DEBUG_CONTROL_PORT = 10

# mach/vm_prot.h

VM_PROT_READ = 0x01
VM_PROT_WRITE = 0x02
VM_PROT_EXECUTE = 0x04

VM_PROT_DEFAULT = VM_PROT_READ | VM_PROT_WRITE

VM_PROT_ALL = VM_PROT_READ | VM_PROT_WRITE | VM_PROT_EXECUTE

# mach/vm_region.h

VM_REGION_BASIC_INFO_64 = 9

# mach/vm_statistics.h

VM_FLAGS_FIXED = 0x00000000
VM_FLAGS_ANYWHERE = 0x00000001
VM_FLAGS_PURGABLE = 0x00000002

# xpc/connection.h

XPC_CONNECTION_MACH_SERVICE_LISTENER = 1 << 0
